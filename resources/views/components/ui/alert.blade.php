@props([
    'type' => 'info',
    'message' => null,
])

@php
$config = match($type) {
    'success' => [
        'bg' => 'bg-green-50 dark:bg-green-950/30',
        'border' => 'border-green-500 dark:border-green-800',
        'text' => 'text-green-800 dark:text-green-300',
        'icon' => 'M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z',
        'title' => '¡Éxito!',
    ],
    'error' => [
        'bg' => 'bg-red-50 dark:bg-red-950/30',
        'border' => 'border-red-500 dark:border-red-800',
        'text' => 'text-red-800 dark:text-red-300',
        'icon' => 'M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z',
        'title' => 'Error',
    ],
    'warning' => [
        'bg' => 'bg-amber-50 dark:bg-amber-950/30',
        'border' => 'border-amber-500 dark:border-amber-800',
        'text' => 'text-amber-900 dark:text-amber-300',
        'icon' => 'M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z',
        'title' => 'Advertencia',
    ],
    'info' => [
        'bg' => 'bg-sky-50 dark:bg-sky-950/30',
        'border' => 'border-sky-500 dark:border-sky-800',
        'text' => 'text-sky-900 dark:text-sky-300',
        'icon' => 'M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zm-1 8a1 1 0 100 2h.01a1 1 0 100-2H10z',
        'title' => 'Información',
    ],
    default => [
        'bg' => 'bg-gray-50 dark:bg-slate-900/60',
        'border' => 'border-gray-500 dark:border-slate-700',
        'text' => 'text-gray-800 dark:text-slate-200',
        'icon' => 'M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zm-1 8a1 1 0 100 2h.01a1 1 0 100-2H10z',
        'title' => 'Información',
    ],
};
@endphp

<div class="mb-4 {{ $config['bg'] }} border {{ $config['border'] }} {{ $config['text'] }} px-4 py-4 rounded-xl shadow-sm flex items-start">
    <svg class="w-5 h-5 mr-3 mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
        <path fill-rule="evenodd" d="{{ $config['icon'] }}" clip-rule="evenodd"/>
    </svg>
    <div>
        <p class="font-medium">{{ $config['title'] }}</p>
        <p class="text-sm mt-1">{{ $message }}</p>
    </div>
</div>
